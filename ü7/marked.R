...
print(head(dwd_raw)) #outputs that are printed using cat() or print() but not stored as objects.
...
print(summary(dwd)) #outputs that are printed using cat() or print() but not stored as objects.

cat("### Missing values per column:\n")
print(colSums(is.na(dwd))) #outputs that are printed using cat() or print() but not stored as objects.

dwd <- dwd %>% filter(date >= as.Date("2010-01-01")) #Identify at least three places where data are transformed multiple times or overwritten.
...
print(summary(dwd$t_mean)) #outputs that are printed using cat() or print() but not stored as objects.

cat("### Quartiles of mean temperature:\n")
print(quantile(dwd$t_mean, probs = c(0.25, 0.5, 0.75), na.rm = TRUE)) #outputs that are printed using cat() or print() but not stored as objects.

############################################################
## 5. Seasonal patterns: boxplots by month & season
############################################################

dwd <- dwd %>% #Identify at least three places where data are transformed multiple times or overwritten.
  mutate(
    month = factor(
      month(date),
      levels = 1:12,
...
print(summary(dwd$precip)) #outputs that are printed using cat() or print() but not stored as objects.
...
dwd <- dwd %>% mutate(wet = precip > 0) #Identify at least three places where data are transformed multiple times or overwritten.

cat("### Wet vs dry days:\n")
print(table(dwd$wet)) #outputs that are printed using cat() or print() but not stored as objects.
print(prop.table(table(dwd$wet))) #outputs that are printed using cat() or print() but not stored as objects.
# base R tools (e.g. table(), prop.table()) are used instead of tidy alternatives.
...
print(cor(dwd$sun, dwd$t_mean, use = "complete.obs")) #outputs that are printed using cat() or print() but not stored as objects.

model_temp_sun <- lm(t_mean ~ sun, data = dwd)

cat("### Regression: t_mean ~ sun\n")
print(summary(model_temp_sun)) #outputs that are printed using cat() or print() but not stored as objects.
...
print(tab_wet_season) #outputs that are printed using cat() or print() but not stored as objects.

cat("### Relative frequencies (within season):\n")
print(prop.table(tab_wet_season, margin = 2)) #outputs that are printed using cat() or print() but not stored as objects.
#base R tools (e.g. table(), prop.table()) are used instead of tidy alternatives.
