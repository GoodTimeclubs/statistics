...
l <- list(vector = x, name = name, Matrix = M)
...
```{r}
data <- data.frame(x = rnorm(10,mean = 5, sd=2), y= rnorm(10,mean = 10, sd = 3))

fp <- tempfile (fileext = ".csv")

write.csv(data,fp,row.names = F)
reading <- read.csv(fp)
reading

plot(reading )
abline(lm(y~x,data = reading))
...
```{r}
n <- 100
x <- rnorm(n) #Was bedeutet das (0,1)?
e <- rnorm(n)
