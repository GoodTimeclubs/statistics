...
vals <- sapply(subs,anonymous)
names(vals) <- subs

vals

events_p_eq_half <- names(vals[vals >= (1/2) - 1e-12 & vals <= (1/2) + 1e-12])

all <- list(P_12,P_all,events_p_eq_half)
names(all) <- c("the probability of {1,2}","the total probability","a subset of vals selected using logical indexing, keeping only those events where vals = 0.5")
all
```
...
```{r 3}
N = 1e5
sim <- mean(rowSums(matrix(sample(1:6,2*N,T),ncol= 2,byrow = F))==10)
exact <- 3/(6*6) #drei günstige aus 36 möglichen

c(Simulated = sim, Calculated= exact)

```

# 4) Exercise 1.4.13

Five-card hand from a 52-card deck. Probability of: (a) four aces plus the
king of spades; (b) all spades; (c) no pairs (all values distinct); (d) full house.
...
```{r 5}
pOfSameBirthday<- function(C,N){
  print(c("currently at ", C ,"Days"))
  mean(
    replicate(N,
            any(
              duplicated(
...
#pOfSameBirthday(23,1e5)

#The crossover is at 23 days

Cvals <- 1:365
rate <- sapply(Cvals,pOfSameBirthday, N=100000)
cross <- min(which(rate > 0.5 & rate < 0.5+0.09))
c(Birthdays = Cvals[cross], P = rate[cross])
