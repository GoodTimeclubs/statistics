...
P_12  <- sum(p[c("1","2")])
P_all <- sum(p)
subs <- list(c(), "1","2","3", c("1","2"),c("1","3"),c("2","3"), c("1","2","3"))
vals <- sapply(subs, function(A) if(length(A)) sum(p[A]) else 0)
names(vals) <- c("∅","{1}","{2}","{3}","{1,2}","{1,3}","{2,3}","Ω")
list(P_12=P_12, P_all=P_all, events_P_eq_half=vals[abs(vals-.5)<1e-12])
```
...
```{r}
N <- 1e5
x <- sample(1:100, N, TRUE)
est <- mean(x%%3==0 | x%%5==0)
exact <- (floor(100/3)+floor(100/5)-floor(100/15))/100
c(estimate=est, exact=exact)
```
...
p_exact <- 3/36
N <- 1e6
s <- rowSums(matrix(sample(1:6, 2*N, TRUE), ncol=2))
p_mc <- mean(s==10)
c(exact=p_exact, mc=p_mc)
```
...
```{r}
total <- choose(52,5)
p_all_spades <- choose(13,5)/total
p_no_pairs   <- (choose(13,5) * 4^5) / total
p_full_house <- (choose(13,1)*choose(4,3) * choose(12,1)*choose(4,2)) / total
c(all_spades=p_all_spades, no_pairs=p_no_pairs, full_house=p_full_house)
```
...
```{r}
prob_match <- function(C, N=200){
  anydup <- function(v) any(duplicated(v))
  mean(replicate(N, anydup(sample(1:365, C, TRUE))))
}
Cs <- 2:60
pm <- sapply(Cs, prob_match)
Cs[which(pm>.5)[1]]
```
...
```{r}
N <- 1e6
X <- matrix(sample(c("H","T"), 3*N, TRUE), ncol=3)
E <- rowSums(X=="H")==2
mean(X[E,1]=="H")
```
...
```{r}
p_blue <- 0.5*(2/5) + 0.5*(7/11)
p_exact <- (0.5*(7/11)) / p_blue
N <- 1e6
urn <- sample(1:2, N, TRUE)
draw_blue <- ifelse(
  urn==1,
  sample(c(rep("B",2),rep("R",3)), N, TRUE),
  sample(c(rep("B",7),rep("R",4)), N, TRUE)
)
mean(urn==2 & draw_blue=="B")/mean(draw_blue=="B")
```
...
c(
  A_and_C = mean(A & C),
  A_times_C = mean(A)*mean(C),
  C_and_D = mean(C & D),
  C_times_D = mean(C)*mean(D)
)
```
...
```{r}
k <- 0:5
p_exact <- dhyper(k, m=8, n=12, k=5)
N <- 2e5
reds <- replicate(N, sum(sample(c(rep(1,8), rep(0,12)), 5)))
p_mc <- table(factor(reds, levels=k))/N
cbind(k, exact=round(p_exact,6), mc=round(as.numeric(p_mc),6))
```
...
pick  <- sample(1:3, N, TRUE)
open <- integer(N)
same <- prize==pick
open[same] <- vapply(prize[same], function(pr) sample(setdiff(1:3, pr), 1), integer(1))
open[!same] <- 6 - prize[!same] - pick[!same]
switch_to <- 6 - pick - open
stay_win   <- mean(pick==prize)
switch_win <- mean(switch_to==prize)
